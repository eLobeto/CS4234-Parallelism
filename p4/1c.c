...
int main(int argc, char *argv[])
{
   ...
   threadlist = malloc(sizeof(pthread_t)*numthreads);
   for(i = 0; i < numthreads; i++){
	  int* pam = malloc(sizeof(int)*5);
    pam[1] = myid;
    pam[2] = dim;
    pam[3] = numcol;
    pam[4] = count;
  	...
  	pthread_create(&threadlist[i],NULL,multiply,parm);
   }
...
void* multiply(void* arg){
  ...
  int k, j , i, counter;
  int threadnum = parm->par[0];
  int myid = parm->par[1];
  int dim = parm->par[2];
  int numcol = parm->par[3];
  int count = parm->par[4];
